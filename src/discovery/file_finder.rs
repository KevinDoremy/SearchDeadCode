    ...
    fn scan_directory(&self, dir: &Path) -> Vec<SourceFile> {
        ...
        walker
            ...
            .filter_map(|entry| {
                ...
                // Committed generated code lies about who uses what: its
                // references prove no human intent and its declarations
                // are not ours to clean up — keep it out of the graph
                if file_type.is_source() && has_generated_header(path) {
                    trace!("Excluding generated file: {}", path.display());
                    return None;
                }
...
/// Standard generated-code markers, matched case-insensitively in the
/// first few lines only — a stray "generated" mid-file is not a header.
